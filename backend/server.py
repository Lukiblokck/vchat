...
from config import SECRET_KEY, DEFAULT_ROOM, PORT, ROOMS
...
@app.route("/api/rooms")
def get_rooms():
    return jsonify({"rooms": ROOMS, "default": DEFAULT_ROOM})
...
def on_connect():
    ...
    sid = request.sid
    room_manager.join_room(sid, DEFAULT_ROOM, username)
    sio_join(DEFAULT_ROOM)
    history = get_recent_messages(DEFAULT_ROOM, limit=50)
    emit("history", {"messages": history, "room": DEFAULT_ROOM})
    emit("user_joined", {"username": username}, to=DEFAULT_ROOM)
    emit("users_update", {"users": room_manager.get_users_in_room(DEFAULT_ROOM)}, to=DEFAULT_ROOM)
...
def on_switch_room(data):
    ...
    if "username" not in session:
        return
    new_room = data.get("room", DEFAULT_ROOM)
    ...
    print(f"[~] {username} cambió a #{new_room}")
